...
const ControlContainer = tw.div`absolute bottom-0 right-0 bg-green-100 px-6 py-4 rounded-tl-3xl border`;
const ControlButton = styled(PrimaryButton)`
  ${tw`mx-3 rounded-full text-green-100 p-2`}
  svg {
    ${tw`w-5 h-5`}
  }
`;
...
const CustomerName = tw.h5`font-semibold text-xl lg:text-2xl xl:text-3xl text-green-500`;
const CustomerTitle = tw.p`font-medium text-secondary-100`;

const QuotesLeft = tw(QuotesLeftIcon)`w-6 h-6 opacity-75 text-green-500 inline-block mr-1 -mt-3`;
const QuotesRight = tw(QuotesRightIcon)`w-6 h-6 opacity-75 text-green-500 inline-block ml-1 -mt-3`;

const DecoratorBlob1 = tw(
  SvgDecoratorBlob1
)`absolute w-32 top-0 left-0 -z-10 text-green-500 opacity-25 transform -translate-x-full`;
const DecoratorBlob2 = tw(
  SvgDecoratorBlob2
)`absolute w-32 bottom-0 right-0 -z-10 text-green-500 opacity-15 transform translate-x-2/3 translate-y-8`;

const Two = ({
  subheading = "",
  heading = "Comentarios",
  description = "Estos son algunos comentarios de nuestros fantasticos clientes sobre su estadia en el complejo vacacional El Algarrobo.",
  testimonials = null,
  textOnLeft = false
}) => {
  /*
  ...
  const defaultTestimonials = [
    {
      imageSrc:
        "https://fotos.alquilerargentina.com/v7/propiedades/hl34/o_hl34_unidad_1VGUNTV6.jpg?p=galeria_lg",
      profileImageSrc:
    ...
    {
      imageSrc:
        "https://fotos.alquilerargentina.com/v7/propiedades/hl34/o_hl34_unidad_GQJCRMMT.jpg?p=galeria_lg",
      profileImageSrc:
